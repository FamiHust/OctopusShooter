...
namespace UnityEngine.UI.Extensions
{
    ...
    public class UIParticleSystem : MaskableGraphic
    {
        ...
        // Tracks live particle count so we can stop dirtying the canvas every frame when idle.
        private int _lastParticleCount = -1;
        private UIVertex[] _quad = new UIVertex[4];
        // Reused local-corner buffer for 3D-rotated particles; avoids a new Vector3[4] per particle per frame.
        private readonly Vector3[] _corners3d = new Vector3[4];
            ...
            for (int i = 0; i < count; ++i)
            {
                ...
                else
                {
                    if (use3dRotation)
                    {
                        ...
                        _corners3d[0] = new Vector3(-size, -size, 0);
                        _corners3d[1] = new Vector3(-size, size, 0);
                        _corners3d[2] = new Vector3(size, size, 0);
                        _corners3d[3] = new Vector3(size, -size, 0);

                        Quaternion particleRotation = Quaternion.Euler(particle.rotation3D);

                        _quad[0].position = pos3d + particleRotation * _corners3d[0];
                        _quad[1].position = pos3d + particleRotation * _corners3d[1];
                        _quad[2].position = pos3d + particleRotation * _corners3d[2];
                        _quad[3].position = pos3d + particleRotation * _corners3d[3];
                    }
                    else
                    {
                        // apply rotation
        ...
        private void Update()
        {
            if (!fixedTime && Application.isPlaying)
            {
                ...
                // Skip the (canvas-wide) mesh rebuild when there is nothing to draw.
                // Keep one extra dirty when transitioning to 0 so the last frame clears the mesh.
                if (pSystem.particleCount > 0 || _lastParticleCount != 0)
                    SetAllDirty();
        ...
        private void LateUpdate()
        {
            ...
            else
            {
                ...
                {
                    pSystem.Simulate(Time.unscaledDeltaTime, false, false, true);
                    // Skip the canvas-wide rebuild when idle; keep one extra dirty to clear the last frame.
                    if (pSystem.particleCount > 0 || _lastParticleCount != 0)
                        SetAllDirty();
                    if ((currentMaterial != null && currentTexture != currentMaterial.mainTexture) ||
                        (material != null && currentMaterial != null && material.shader != currentMaterial.shader))
